 ...
 *
 * Named store.mjs rather than playlist.mjs so Netlify treats it as a brand new
 * function and cannot reuse an older cached bundle.
 *
...
async function store(){
  if(!_getStore){
    ...
    catch(e){ throw new Error("blobs package not bundled: " + (e?.message || e)); }
    _getStore = mod?.getStore;
    if(typeof _getStore !== "function") throw new Error("getStore missing from @netlify/blobs");
  }
  // Signatures have changed across versions; take whichever one works.
  const attempts = [
    () => _getStore({ name: "playlist", consistency: "strong" }),
    () => _getStore({ name: "playlist" }),
    () => _getStore("playlist")
  ];
  let last;
  ...
  throw new Error("could not open store: " + (last?.message || "unknown"));
}
